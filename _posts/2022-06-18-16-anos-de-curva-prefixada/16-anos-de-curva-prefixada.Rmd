...
cache_folder <- "C:\\Users\\wilso\\OneDrive\\Documentos\\rb3-cache"
df <- futures_mget(
...
futuros.
O problema é que o data.frame é bastante grande e utilizar a função `rb3::maturity2date`
para converter o código de vencimento na data de vencimento leva um tempão.
Por isso vou dar um `distinct` na coluna `maturity_code` para obter as datas de vencimento e 
na sequência vou fazer um *join* com o data.frame original.

```{r cache=TRUE}
mats <- df_di1 |>
  distinct(maturity_code) |>
  mutate(maturity_date = maturity2date(maturity_code))
```

Aqui utilizo o `dplyr::left_join` para juntar os vencimentos ao data.frame original.
...
df_di1_futures <- df_di1 |>
  left_join(mats, by = "maturity_code") |>
  mutate(
